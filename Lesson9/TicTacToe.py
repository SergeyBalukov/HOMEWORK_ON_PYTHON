...
# Вложенный цикл для отрисовки квадратиков
    if not game_over:
        for row in range(3):
            for col in range(3):
                if mas[row][col] == 'x':
    ...
    if game_over:
        screen.fill(black)
        # создаем шрифт (название'stxingkai'и размер 80)
        font = pygame.font.SysFont('stxingkai', 80)
        text1 = font.render(game_over, True, white)
        text_rect = text1.get_rect()
        # Находим центр экрана, что бы текст распологался по центру
        text_x = screen.get_width() / 2 - text_rect.width / 2
        text_y = screen.get_height() / 2 - text_rect.height / 2
        # Прикрепляет наш текст по найденным координатам
